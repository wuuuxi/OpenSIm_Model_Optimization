...
def plot_picture(mif, errors):
    ...
    plt.figure(figsize=(6, 5))
    rmse_BIC = [np.sqrt(np.sum((errors[s, :, 0]) ** 2) / errors.shape[1]) for s in range(errors.shape[0])]
    rmse_BRA = [np.sqrt(np.sum((errors[s, :, 1]) ** 2) / errors.shape[1]) for s in range(errors.shape[0])]
    rmse_BRD = [np.sqrt(np.sum((errors[s, :, 2]) ** 2) / errors.shape[1]) for s in range(errors.shape[0])]
    plt.subplot(311)
    plt.plot(rmse_BIC)
    plt.subplot(312)
    plt.plot(rmse_BRA)
